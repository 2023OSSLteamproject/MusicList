...
void readMusic(Music m); //상단바에서 보여주는 모든 것을 읽는 함수
int addMusic(Music *m); //음악을 추가하는 함수
void listMusic(Music *m[], int count); //현 음악리스트를 보여주는 함수
void deleteMusic(Music *m[], int count); //음악을 플레이리스트에서 삭제하는 함수
void updateMusic(Music *m); //플레이리스트를 수정하는 함수
